...
*/   
   ...
   while(LeoBajas(&lNroCliente)){
      GeneraBaja(lNroCliente);
...
short AbreArchivos()
{
   ...
   sprintf(sArchBajasDos, "%sEnel_care_MOVEOUT_%s_%s.csv", sPathSalida, gsDesdeFmt, gsHastaFmt);	
   ...
   strcpy(sTitulos, "\"Identificador PoD\";\"Estado Suministro\"\n");
...
$char sClave[7];
   ...
   iRcv=system(sCommand);
	...
	sprintf(sCommand, "cp %s %s", sArchBajasDos, sPathCp);
	iRcv=system(sCommand);
...
$char sAux[1000];
   ...
   /******** Bajas Puras ********/
   strcpy(sql, "INSERT INTO sf_actuclie (numero_cliente, baja) "); 
...
*/   
   ...
   strcat(sql, "WHERE m.fecha_modif BETWEEN ? AND ? ");
   strcat(sql, "AND m.tipo_orden = 'MOD' ");
   strcat(sql, "AND m.codigo_modif IN ('2','76','77','21','60','64','94','269') ");

   $PREPARE insVarios FROM $sql;
