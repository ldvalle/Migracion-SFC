...
*/   
  ...
  fp=pFileMedidorUnx;
	...
	while(LeoMedidores(&regMedidor)){
      if(!CargaDataInstal(&regMedidor)){
         printf("Fallo CargaDataInstal\n");
         exit(1);
      }
		if (!GenerarPlano(fp, regModif, regMedidor, "I")){
         printf("Fallo GenearPlano\n");
			exit(1);	
		}
...
short AbreArchivos()
{
   ...
   FechaGeneracionFormateada(sFecha);
	...
	sprintf( sArchMedidorUnx  , "%sT1DEVICE_INICIAL.unx", sPathSalida );
   sprintf( sArchMedidorAux  , "%sT1DEVICE_INICIAL.aux", sPathSalida );
   sprintf( sArchMedidorDos  , "%senel_care_device_inicial_t1_%s_%s.csv", sPathSalida, gsDesdeFmt, gsHastaFmt);
   ...
   strcpy(sTitulos,"\"Marca Medidor\";\"Modelo Medidor\";\"Nro.Medidor\";\"Propiedad Medidor\";\"Tipo Medidor\";\"Punto de Suministro\";\"External ID\";\"Estado Medidor\";\"Fecha Ult.Instalacion\";\"Constante\";\"Fecha Fabricacion\";\"Fecha Retiro\";\"Lectura Instalacion\";\"Lectura Retiro\";\n");
...
if(giTipoCorrida == 1){
   strcat(sql, "AND ma.numero_cliente = m.numero_cliente ");
	...
	$PREPARE selMed FROM $sql;
	$DECLARE curMedidores CURSOR FOR selMed;
	...
	/** lECTURA Instalacion **/
	$PREPARE selLectuInstal FROM "SELECT h1.fecha_lectura, 
		TO_CHAR(h1.fecha_lectura, '%Y-%m-%d'), 
		h1.lectura_facturac
		FROM hislec h1
		WHERE h1.numero_cliente = ?
		AND h1.numero_medidor = ?
		AND h1.marca_medidor = ?
		AND h1.tipo_lectura IN (6, 7)	";

	$PREPARE selLectuInstal2 FROM "SELECT h1.fecha_lectura, 
		TO_CHAR(h1.fecha_lectura, '%Y-%m-%d'), 
		h1.lectura_facturac
		FROM hislec h1
		WHERE h1.numero_cliente = ?
  	AND h1.tipo_lectura IN (1,2,3,4)
    AND h1.fecha_lectura = (SELECT MIN(h2.fecha_lectura) FROM hislec h2
   ...
   $PREPARE selUltimaLectu FROM "SELECT h.corr_facturacion, h.tipo_lectura, h.lectura_facturac FROM cliente c, hislec h
...
$ClsMedidor	*regMed;
{
   ...
   memset(regMed->fecha_ult_insta, '\0', sizeof(regMed->fecha_ult_insta));
	...
	rsetnull(CDOUBLETYPE, (char *) &(regMed->lecturaInstal));
   rsetnull(CLONGTYPE, (char *) &(regMed->lFechaInstalacion));
   memset(regMed->sFechaInstalacion, '\0', sizeof(regMed->sFechaInstalacion));
   rsetnull(CDOUBLETYPE, (char *) &(regMed->lecturaInstalacion));
	
}

short CargaDataInstal(reg)
$ClsMedidor *reg;
{

	$EXECUTE selLectuInstal INTO :reg->lFechaInstalacion,
			:reg->sFechaInstalacion,
			:reg->lecturaInstalacion
		USING :reg->numero_cliente,
			:reg->numero,
			:reg->marca;

	if(SQLCODE != 0){
		$EXECUTE selLectuInstal2 INTO :reg->lFechaInstalacion,
				:reg->sFechaInstalacion,
				:reg->lecturaInstalacion
			USING :reg->numero_cliente;
		
		if(SQLCODE !=0 ){
			printf("No se pudo encontrar lectura de instalacion para cliente %ld\n", reg->numero_cliente);
			strcpy(reg->sFechaInstalacion, reg->fecha_ult_insta);
			reg->lecturaInstalacion = reg->lecturaInstal;			
		}
	}
...
char           accion[2];
{
   ...
   /* Nro.Medidor */
   sprintf(sLinea, "%s\"%09ld\";", sLinea, regMed.numero);
...
*/
   ...
   /* External ID */
   sprintf(sLinea, "%s\"%ld%09ld%s%sDEVARG\";", sLinea, regMed.numero_cliente, regMed.numero, regMed.marca, regMed.modelo);
...
*/
   ...
   sprintf(sLinea, "%s\"%s\";", sLinea, regMed.sFechaInstalacion);
   ...
   /* Lectura Instalacion */
   if(!risnull(CDOUBLETYPE, (char *) &regMed.lecturaInstalacion)){
		sprintf(sLinea, "%s\"%.0f\";", sLinea, regMed.lecturaInstalacion);
	}
...
char           accion[2];
{
   ...
   /* Lectura Instalacion */
   if(!risnull(CDOUBLETYPE, (char *) &regCT.lecturaInstalacion)){
		sprintf(sLinea, "%s\"%.0f\";", sLinea, regCT.lecturaInstalacion);
	}else{
		strcat(sLinea, "\"\";");
	}
		
   /* Lectura Retiro */
   if(!risnull(CDOUBLETYPE, (char *) &regCT.lecturaRetiro)){
		sprintf(sLinea, "%s\"%.0f\";", sLinea, regCT.lecturaRetiro);
	}else{
		strcat(sLinea, "\"\";");
	}
